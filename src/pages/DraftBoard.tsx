...
const BOARD_POSITIONS = ['QB', 'RB', 'WR', 'TE', 'K']
...
const DEFAULT_SHAPE: LeagueShapeInput = { teams: 12, qb: 1, rb: 2, wr: 3, te: 1, flex: 1 }

export default function DraftBoard() {
  const [shape, setShape] = useLocalStorage<LeagueShapeInput>('ark.leagueShape.v1', DEFAULT_SHAPE)
  ...
  const needs = BOARD_POSITIONS.map((position) => {
    const required =
      position === 'QB' ? shape.qb
      : position === 'RB' ? shape.rb
      : position === 'WR' ? shape.wr
      : position === 'TE' ? shape.te
      : 1
    return {
      position,
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-3 grid-cols-2 lg:grid-cols-4">
        ...
        <StatTile
          ...
          value={cliffPosition(available)}
          hint="Where the next tier falls away fastest"
        />
      </div>

      <div className="grid gap-4 lg:grid-cols-3">
        <Card title="Best available" subtitle="Ranked by value over replacement" className="lg:col-span-1">
          {matching.length === 0 ? (
      ...
      <p className="px-1 text-xs leading-relaxed text-ink-500">
        ...
        support it, then adjusted by measured age curves and depth chart position. Kicker scoring is
        computed from field goals by distance. Team defences are not ranked — the open data does not
        carry defensive fantasy scoring, and a made-up number would be worse than none. Nothing here
...
function cliffPosition(available: DraftPoolPlayer[]): string {
